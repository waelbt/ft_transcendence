...
export class friendsController {
    ...
    @ApiParam({
        name: 'friendId',
        description: 'id of the user recieves friend request',
        type: 'string'
    })
    ...
    @ApiParam({
        name: 'friendId',
        description: 'id of the user that send a friend request',
        type: 'string'
    })
    async acceptFriendRequest(@Req() req, @Param('friendId') friendId: string) {
        try {
    ...
    async rejectFriendRequest(@Req() req, @Param('friendId') friendId: string) {
        try {
    ...
    @Get('typeProfile/:id2')
    async typeOfProfile(@Req() req, @Param('id2') userId2: string) {
    ...
    async numberOfFriends(@Req() req) {
        try {
            const numberOfFriends =
                await this.friendsService.getNumberOfFriends(req.user.sub);
    ...
    async userListFriends(@Req() req, @Param('viewerId') viewerId: string) {
        try {
