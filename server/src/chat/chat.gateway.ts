    ...
    implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
    ...
    async handleConnection(client: any, ...args: any[]) {
        ...
        if (userCheck.state === true) {
            ...
            await this.wsService.joinUserSocketToGlobalChat(
                client.id,
                this.server
            );
        }
    ...
    async joinRoom(client: Socket, joinRoomDto: JoinRoomDto) {
        ...
        this.logger.log(
            `the user  ${joinRoomDto.id} is trying to join the room "${joinRoomDto.roomId}"`
        );
    ...
    async leaveRoom(client: Socket, leaveRoomDto: LeaveRoomDto) {
        ...
        //   throw new WsException(userCheck.message);
        console.log(
            `The user ${leaveRoomDto.id} is leaving the room ${leaveRoomDto.roomTitle}`
        );
    ...
    async globalChat(client: Socket, payload: CreateMessageDto) {
        console.log('wael   ', client.handshake.auth.token);
        const userCheck = await this.wsService.getUserFromAccessToken(
          ...
          console.log('wael   ', payload.message);
    }
